...
resource "confluent_environment" "demo" {
  display_name = "Demo_Real_Time_Data_Warehousing"
}
...
# 'app-manager' service account is required in this configuration to create 'postgres.customers.customers' topic and grant ACLs
...
# Create postgres.customers.customers topic 
resource "confluent_kafka_topic" "postgres_customers_customers" {
  kafka_cluster {
    id = confluent_kafka_cluster.basic.id
  }

  topic_name       = "postgres.customers.customers"
...
# Create postgres.customers.demographics topic 
resource "confluent_kafka_topic" "postgres_customers_demographics" {
  kafka_cluster {
    id = confluent_kafka_cluster.basic.id
  }

  topic_name       = "postgres.customers.demographics"
  rest_endpoint    = confluent_kafka_cluster.basic.rest_endpoint
  partitions_count = 1
  credentials {
    key    = confluent_api_key.app-manager-kafka-api-key.id
...
resource "aws_security_group" "postgres_sg" {
    name = "rtdw_postgres_security_group_${split("-", uuid())[0]}"
    description = "Security Group for Postgres EC2 instance. Used in Confluent Cloud Realtime Data Warehousing workshop."
